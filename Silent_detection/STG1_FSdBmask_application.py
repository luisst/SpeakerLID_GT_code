...
def calculate_rms_segments_db(audio_file,
    ...
    mask_bool = full_scale_rms_db < -24
    ...
    if verbose:
        ...
        audio_duration_from_segments = len(full_scale_rms_db)*0.2
...
for current_gt_pth, current_pred_pth in matches_for_masking:
    ...
    for i in range(len(vad_df)):
        ...
        high_db_segments = split_segments(start_time, end_time, mask_segment, 3)
    ...
    current_output_csv_path = output_csv_folder / f'{current_gt_pth.stem}_filtered.csv'
    ...
    print(f'Wrote to CSV path: {current_output_csv_path}\n')
