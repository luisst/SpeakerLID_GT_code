...
def extract_ids(input_href):
    ...
    id_list = []
    pattern = re.compile(r'\b([A-Za-z0-9]+\.[A-Za-z0-9]+\.\d+)\b')
...
def process_segment(segment, word_dict, segs_file, data):
    ...
    starttime = segment.attrib.get("starttime", "")
    endtime = segment.attrib.get("endtime", "")
    ...
    data.append([participant, starttime, endtime, transcription])
...
def process_session(segs_file, words_file, output_csv):
    ...
    for segment in root:
        if segment.get("type") == "supersegment":
            # Extract child segments and treat them as regular segments
            for child in segment.findall(".//segment"):
                data, word_dict = process_segment(child, word_dict, segs_file, data)
        else:
...
def process_directory(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".segs.xml"):
            base_name = filename.replace(".segs.xml", "")
...
# Example usage
input_directory = Path(r'C:\Users\luis2\Dropbox\DATASETS_AUDIO\ICSI\input_segs_words')
output_directory = input_directory.joinpath('my_output_separated')
process_directory(str(input_directory), str(output_directory))

#TODO: Add a function to process the output files and merge the ones with the same substring code. i.e. 'Bdb001'
