...
current_folder = Path.home().joinpath('Dropbox','DATASETS_AUDIO','AOLME_SD_Collection','riogrande','Tuesday', 'Ugesh')
output_wav_folder = current_folder.joinpath('wav_output')

if not(check_folder_for_process(output_wav_folder)):
    sys.exit('goodbye')

for mp4_pth in current_folder.glob( '*.wav' ):
    print(f'\n This is the mp4: {mp4_pth.name}\n---------------------\n')
